...
const char* deviceLocation = "Pat's current location";
...
const char* ssid     = "PAT";  // SSID of the ESP32 access point
...
struct LogEntry {
  ...
  float pres;                  // Pressure value in hPa
...
<head>
  <meta charset="UTF-8">
  <title>ESP32 BME280 Monitor</title>
</head>
<body>
  <h1 id="loc">Location: (loading...)</h1>

  <h2>Temperature, Humidity & Pressure</h2>

  <p>Temperature: <span id="temp">--</span> °C</p>
  <p>Humidity:    <span id="hum">--</span> % RH</p>
  <p>Pressure:    <span id="pres">--</span> kPa</p>
    ...
    function update() {
      fetch('/data')
        ...
        .then(d => {
          ...
          document.getElementById('pres').textContent = d.kpa.toFixed(2);
          if (d.location) {
            document.getElementById('loc').textContent = 'Location: ' + d.location;
          }
        })
...
/*
  ...
  Example line:
  #12 t=12345 ms  T=22.5 C  H=41.2 %  P=1008.3 hPa  | IP: 192.168.4.1
*/
void printSerialLine(unsigned long now, float t, float p, float h) {
  ...
  if (now - lastIpPrint >= 5000) {
    ...
    Serial.print("  | IP: ");
    Serial.print(ip);            // e.g. 192.168.4.1
    lastIpPrint = now;
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== ESP32 BME280 Monitor ===");
